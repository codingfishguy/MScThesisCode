...
def plot_credible_regions(theta_star, data,
                           abc_region, npe_region,
                           globconf_posterior, globconf_cutoff,
                           robconf_posterior, robconf_cutoff, robconf_theta_train,
                           figsize=(15, 5), robnpe_region=None):
    """
    Each of abc_region / npe_region / robnpe_region is (mu, Sigma, threshold),
    as returned by abc_confregion / npe / robnpe.

    theta_star: the pseudo-true parameter (minimum-distance estimate under the
    misspecified/assumed simulator family, matched via Euclidean distance on
    the full 34-dim data against the true logistic-growth DGP). Plotted as the
    reference point instead of theta_true, since under misspecification the
    assumed-model-based methods (ABC, NPE, RobNPE, GlobConf, RobConf) have no
    way to recover theta_true -- theta_star is what they should concentrate on.
    """
    global obs_data
    ...
    for ax, (i, j) in zip(axes, PARAM_PAIRS):
        ...
        ax.scatter([theta_star[i]], [theta_star[j]], marker='*', s=200,
                   color='black', zorder=5, label=r'$\theta^{*}$')
...
# Data now generated under the TRUE (misspecified) DGP: logistic prey growth, K=100
obs_data = simulate_lv_true(1.0, 0.005, 0.6, K=100, seed=1)
...
fig = plot_credible_regions(
    theta_star=(0.22375171, 0.0071137, 0.56289875), data=obs_data, #found on jupyter notebook. (0.5598802, 0.0072013, 0.64885456) for K=200
    abc_region=abc_region, npe_region=npe_region,
    ...
    robnpe_region=robnpe_region
)
fig.savefig("lv_robconf_extreme_10000_K100_0025_01", dpi=150)
